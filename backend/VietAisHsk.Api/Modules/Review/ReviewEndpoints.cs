...
public static class ReviewEndpoints
{
    public static IEndpointRouteBuilder MapReviewEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapPost("/sessions/{id}/results", (string id, RecordReviewResultRequest request, IUserContextAccessor contextAccessor, IReviewStore store, IProgressSignalSink progressSignals, IReviewClock clock) =>
        {
            if (contextAccessor.Current is null)
            {
                return Results.Unauthorized();
            ...
            progressSignals.RecordActivity(
                contextAccessor.Current.UserId,
                "review-completed",
                $"{id}:{item.Id}",
                item.LastAttemptedAt ?? clock.UtcNow);
            progressSignals.ApplyReviewSignal(new ReviewEvaluationSignal(
                contextAccessor.Current.UserId,
                item.KnowledgeType,
                item.KnowledgeId,
                request.Correct ? "Correct" : "Incorrect",
                item.LastAttemptedAt ?? clock.UtcNow,
                $"review:{id}:{item.Id}"));
