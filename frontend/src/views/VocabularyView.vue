...
function speak(text: string) {
  if (!('speechSynthesis' in window)) {
    error.value = 'Trình duyệt chưa hỗ trợ phát âm thanh.'
    return
  }
...
function syncSelected() {
  ...
  selected.value = typeof id === 'string'
    ? entries.value.find(item => item.id === id || item.simplified === id) ?? null
    : null
}
