...
public sealed class InMemoryReviewStore : IReviewStore
{
    ...
    {
        var itemId = BuildItemId(signal.UserId, signal.KnowledgeType, signal.KnowledgeId, ReviewReason.WrongAnswer);
        ...
        if (string.Equals(signal.Result, "Incorrect", StringComparison.OrdinalIgnoreCase))
        {
            items.AddOrUpdate(
                itemId,
                _ => new ReviewItem(itemId, signal.UserId, signal.KnowledgeType, signal.KnowledgeId, ReviewReason.WrongAnswer, 60, 1, now, now, false),
    ...
    public ReviewSummary GetSummary(string userId)
    {
        ...
        return new ReviewSummary(
            all.Count(item => item.NextReviewAt <= DateTimeOffset.UtcNow),
    ...
    public ReviewSession? StartSession(string userId, IReadOnlyList<string> itemIds)
    {
        ...
        var session = new ReviewSession(Guid.NewGuid().ToString("N"), userId, eligible, DateTimeOffset.UtcNow);
